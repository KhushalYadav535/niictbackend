...
// Test the production result search API
const testProductionAPI = async () => {
  try {
    console.log('Testing Production Result Search API...\n');
    ...
    for (const rollNumber of testRollNumbers) {
      ...
      try {
        const response = await fetch(`https://niictbackend.onrender.com/api/results/search/${rollNumber}`);
    ...
    try {
      const response = await fetch('https://niictbackend.onrender.com/api/results/search/INVALID123');
      const data = await response.json();
